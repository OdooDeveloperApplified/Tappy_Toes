...
class CrmLeadAPI(http.Controller):
    ...
    @http.route('/create-crm-opportunity', type='http', auth='public', methods=['POST'], csrf=False)
    def create_crm_opportunity(self, **post):
        try:
            # Accept JSON OR form-data
            data = post
            if not post:
            ...
            _logger.info("CRM Opportunity Created ID: %s", lead.id)
            ...
            _logger.exception("Error creating CRM Opportunity")
            return json.dumps({
                'status': False,
                'error': str(e)
            })